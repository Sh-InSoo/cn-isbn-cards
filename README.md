...
Phase 1 ─ [NAS / Docker]  중국망에서 NPPA 직접 접근, 매월 22일~ 매일
  ...
  data_export.py    3종 모두 발표된 달의 수치를 JSON으로 → 로컬 저장 + GitHub API push
                          │
...
Phase 3 ─ [NAS / Docker]  다음 daily 실행
  card_publisher.py editorial JSON을 GitHub에서 HTTPS fetch → 있으면:
...
## NAS 측 배포 (git-less HTTPS 핸드오프)

NAS에는 git이 없으므로 핸드오프는 git이 아니라 **HTTPS**로 한다:
- Phase 1 `data_export.export_and_push(...)` — 스크랩 JSON을 `{DATA_DIR}`에 로컬
  저장하고, **GitHub Contents API**로 push(`GITHUB_TOKEN` 필요).
- Phase 3 `card_publisher.publish_cards_if_ready(...)` — 로컬 스크랩 JSON을 읽고,
  routine이 올린 editorial JSON을 **GitHub에서 HTTPS로 fetch**(공개 repo → 무인증).
  받으면 카드를 렌더·업로드한다.
...
- `GITHUB_REPO` / `GITHUB_BRANCH` — 기본 `Sh-InSoo/cn-isbn-cards` / `main`.
